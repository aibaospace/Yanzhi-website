(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))t(o);new MutationObserver(o=>{for(const i of o)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&t(l)}).observe(document,{childList:!0,subtree:!0});function r(o){const i={};return o.integrity&&(i.integrity=o.integrity),o.referrerPolicy&&(i.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?i.credentials="include":o.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function t(o){if(o.ep)return;o.ep=!0;const i=r(o);fetch(o.href,i)}})();const v="https://conference-tracker-production.up.railway.app";function x(){return localStorage.getItem("ct_internal")==="1"}function y(){return localStorage.getItem("ct_token")}function I(n){localStorage.setItem("ct_token",n)}function E(){localStorage.removeItem("ct_token")}async function k(n){const e=await fetch(`${v}/api/auth/me`,{headers:{Authorization:`Bearer ${n}`}});return e.ok?e.json():null}class B extends HTMLElement{connectedCallback(){this.innerHTML=`
    ...
    `,n.innerHTML=`
        ...
        ">
            ...
            <div id="ct-logged-in" style="display:none; padding:1.5rem; text-align:center;">
                ...
                <a id="ct-admin-btn" href="/internal/pr-tool.html" style="display:none; margin-bottom:0.5rem; padding:0.5rem 1.2rem; background:#1e293b; color:#fff; border-radius:6px; cursor:pointer; font-size:0.9rem; text-decoration:none; font-weight:600; display:none;">Admin Panel</a>
    ...
    `,t.addEventListener("mouseenter",()=>{t.style.background="#e8e8e8",t.style.color="#1d4ed8",t.style.textDecoration="underline"}),t.addEventListener("mouseleave",()=>{t.style.background="#f5f5f5",t.style.color="#444",t.style.textDecoration="none"}),t}function g(n){const e=document.getElementById("embodyx-sidebar-logo")||document.querySelector(".university-logo");if(e)if(n){const r=n.role==="admin"||n.role==="owner";r&&!d?(d=f("/pr-tool.html","PR Tool","2rem"),e.insertAdjacentElement("afterend",d)):!r&&d&&(d.remove(),d=null),s||(s=f("/conferences/","Conference Tracker","0.5rem"),(d||e).insertAdjacentElement("afterend",s)),r&&!a?(a=f("/conferences/#news","News Management","0.5rem"),s.insertAdjacentElement("afterend",a)):!r&&a&&(a.remove(),a=null)}else s&&(s.remove(),s=null),a&&(a.remove(),a=null),d&&(d.remove(),d=null)}function p(n){const navLinks=document.querySelector(".nav-links");const existingItem=document.getElementById("u-site-item");if(existingItem)existingItem.remove();else{const old=document.getElementById("users-nav-link");if(old&&old.parentElement&&old.parentElement.tagName==="LI")old.parentElement.remove();}if(!navLinks)return;const li=document.createElement("li");li.id="u-site-item";li.style.position="relative";navLinks.appendChild(li);if(n){const roleColor={owner:"#7c3aed",admin:"#2563eb",user:"#64748b"}[n.role]||"#64748b",roleBg={owner:"#ede9fe",admin:"#dbeafe",user:"#f1f5f9"}[n.role]||"#f1f5f9",isElevated=n.role==="admin"||n.role==="owner";li.innerHTML=`<a href="#" id="users-nav-link">${n.email.split("@")[0]} ▾</a><div id="u-site-drop" style="display:none;position:absolute;right:0;top:calc(100% + 8px);min-width:220px;background:#fff;border:1px solid #e2e8f0;border-radius:10px;box-shadow:0 4px 16px rgba(0,0,0,0.12);z-index:2000;padding:0.375rem 0;"><div style="padding:0.5rem 0.875rem;font-size:0.78rem;border-bottom:1px solid #f1f5f9;margin-bottom:0.25rem;"><div style="font-weight:600;color:#1e293b;font-size:0.85rem;">${n.email}</div><span style="display:inline-block;font-size:0.65rem;font-weight:700;padding:0.1rem 0.4rem;border-radius:4px;margin-top:0.25rem;text-transform:uppercase;background:${roleBg};color:${roleColor};">${n.role||"user"}</span></div>${isElevated?'<a href="/internal/pr-tool.html" style="display:block;padding:0.5rem 0.875rem;font-size:0.84rem;color:#374151;text-decoration:none;">Admin Panel</a>':""}<button id="u-site-logout" style="display:block;width:100%;text-align:left;background:none;border:none;padding:0.5rem 0.875rem;font-size:0.84rem;color:#ef4444;font-family:inherit;cursor:pointer;">Sign Out</button></div>`;const lnk=document.getElementById("users-nav-link"),drop=document.getElementById("u-site-drop");lnk.addEventListener("click",o=>{o.preventDefault();o.stopPropagation();drop.style.display=drop.style.display==="none"?"block":"none";});document.addEventListener("click",()=>{drop.style.display="none";});document.getElementById("u-site-logout").addEventListener("click",()=>{E(),g(null),p(null);});}else{li.innerHTML='<a href="#" id="users-nav-link">Users</a>';document.getElementById("users-nav-link").addEventListener("click",o=>{o.preventDefault(),b();});}}document.addEventListener("DOMContentLoaded",async()=>{L(),setTimeout(()=>{var e;(e=document.getElementById("users-nav-link"))==null||e.addEventListener("click",r=>{r.preventDefault(),b()})},0);const n=y();if(n){const e=await k(n);e?(g(e),p(e)):E()}});
